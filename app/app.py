...
@st.cache_data
def load_data():
     # use the loader that already injects pricing columns from latest MASTER
     return (
         carregar_comparacao_master_com_precos(),
         carregar_amazon_sample(),
         carregar_ml_sample()
...
for _, row in df_filtrado.iterrows():
    ...
    # preços diretamente da comparação MASTER; se não houver, tenta lookup de sample
    preco_am = row.get("Preço Prod Amazon", "")
    if not preco_am:
        asin_amz = row["ASIN Amazon"]
        preco_am = buscar_preco_amazon(asin_amz)

    preco_ml = row.get("Preço Prod ML", "")
    if not preco_ml:
